...
export const CreateDoctor = ({ urlApi,tratamientos,generos }) => {
  ...
  return (
    <div className="App">
      ...
      { alert === 'success' && <Modal Icon={Success} iconColor={'#0f0'} setOpen={setAlert} title={'Médico Registrado'} buttons={1} />  }
      { alert === 'error' && <Modal Icon={Error} iconColor={'#f00'} setOpen={setAlert} title={'Médico No Registrado'} buttons={1} />  }
    </div>
  );
};