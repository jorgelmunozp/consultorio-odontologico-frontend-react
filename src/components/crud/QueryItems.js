    ...
    <div className="App">
      <div className='mt-4 mt-sm-5'>
        ...
        <div className='container-fluid border overflow-auto'>
          <div className={'row flex-nowrap bg-main-color' + (isMenuOpen ? ' ms-2 ms-sm-5 ps-5':'')}>
            ...
            { arrayFiltered.sort(SortByProperty).slice(indexPage[0],indexPage[1]).map((item) => { return (
                <div className={'row flex-nowrap border-bottom text-start text-nowrap py-2' + (isMenuOpen ? ' start-0':'')} id={'row'+item.id } key={ item.id }>
